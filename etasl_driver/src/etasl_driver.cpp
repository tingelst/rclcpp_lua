                                             ...
                                             DoubleMap& result)
{
  ...
  if (solver_->getNrOfFeatureStates() > 0)
  {
    ...
    for (t = 0; t < initialization_time; t += sample_time)
    {
      ...
      {
        // ROS_ERROR_STREAM(
        //     "EtaslDriver::initializeFeatureVariables: Solver encountered the following error during initialization
        //     (t="
        //     << t << ")");
...
EtaslDriver::EtaslDriver(int nWSR, double cputime, double regularization_factor)
{
  ...
  lua_ = std::make_shared<LuaContext>(ctx_);

  solver_ = std::make_shared<qpOASESSolver>(nWSR, cputime, regularization_factor);
...
void EtaslDriver::getOutput(DoubleMap& dmap)
{
  ...
  {
    Expression<double>::Ptr expr = std::dynamic_pointer_cast<Expression<double>>(it->second);
    if (expr)
    {
...
void EtaslDriver::getOutput(FrameMap& fmap)
{
  for (Context::OutputVarMap::iterator it = ctx_->output_vars.begin(); it != ctx_->output_vars.end(); it++)
  {
    Expression<Frame>::Ptr expr = std::dynamic_pointer_cast<Expression<Frame>>(it->second);
    if (expr)
    {
...
void EtaslDriver::getOutput(VectorMap& vmap)
{
  for (Context::OutputVarMap::iterator it = ctx_->output_vars.begin(); it != ctx_->output_vars.end(); it++)
  {
    Expression<Vector>::Ptr expr = std::dynamic_pointer_cast<Expression<Vector>>(it->second);
    if (expr)
    {
...
void EtaslDriver::getOutput(RotationMap& rmap)
{
  for (Context::OutputVarMap::iterator it = ctx_->output_vars.begin(); it != ctx_->output_vars.end(); it++)
  {
    Expression<Rotation>::Ptr expr = std::dynamic_pointer_cast<Expression<Rotation>>(it->second);
    if (expr)
    {
...
void EtaslDriver::getOutput(TwistMap& tmap)
{
  for (Context::OutputVarMap::iterator it = ctx_->output_vars.begin(); it != ctx_->output_vars.end(); it++)
  {
    Expression<Twist>::Ptr expr = std::dynamic_pointer_cast<Expression<Twist>>(it->second);
    if (expr)
    {
                            ...
                            double convergence_crit, DoubleMap& convergedval)
{
  ...
  if (retval != 0)
  {
    // ROS_ERROR_STREAM("EtaslDriver::initialize (prepareExecution): The task specification contains priority levels
    // that "
